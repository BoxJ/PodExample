...
@protocol JMAnimatedImageViewDebugDelegate <NSObject>
@optional
- (void)debug_animatedImageView:(JMAnimatedImageView *)animatedImageView waitingForFrame:(NSUInteger)index duration:(NSTimeInterval)duration;
@end
#endif
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<JMAnimatedImageViewDebugDelegate> debug_delegate;
#endif
...
- (void)setAnimatedImage:(JMAnimatedImage *)animatedImage {
    if (![_animatedImage isEqual:animatedImage]) {
        if (animatedImage) {
            if (super.image) {
                ...
                // See more here: https://github.com/Flipboard/JMAnimatedImage/issues/100
...
- (NSTimeInterval)frameDelayGreatestCommonDivisor {
    // Presision is set to half of the `kJMAnimatedImageDelayTimeIntervalMinimum` in order to minimize frame dropping.
    const NSTimeInterval kGreatestCommonDivisorPrecision = 2.0 / kJMAnimatedImageDelayTimeIntervalMinimum;
...
- (void)startAnimating {
    if (self.animatedImage) {
        ...
        if (!self.displayLink) {
            ...
            self.displayLink = [CADisplayLink displayLinkWithTarget:[self weakProxy] selector:@selector(displayDidRefresh:)];
...
- (void)displayDidRefresh:(CADisplayLink *)displayLink {
    ...
    // Early return!
    if (!self.shouldAnimate) {
        JMLog(@"Trying to animate image when we shouldn't: %@", self);
        return;
    }
    ...
    if (delayTimeNumber != nil) {
        ...
        if (image) {
            JMLog(@"Showing frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
            ...
            JMLog(@"Waiting for frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
#if defined(DEBUG) && DEBUG
