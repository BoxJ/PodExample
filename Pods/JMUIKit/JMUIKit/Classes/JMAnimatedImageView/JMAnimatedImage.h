...
extern const NSTimeInterval kJMAnimatedImageDelayTimeIntervalMinimum;

//
//  An `JMAnimatedImage`'s job is to deliver frames in a highly performant way and works in conjunction with `JMAnimatedImageView`.
...
//
@interface JMAnimatedImage : NSObject
...
// Pass either a `UIImage` or an `JMAnimatedImage` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `JMAnimatedImage` with all fields initialized, on failure they return `nil` and an error will be logged.
