...
// JMWebViewJavascriptBridge. It is plaintext, but converted to an NSString
// via some preprocessor tricks.
//
// Previous implementations of JMWebViewJavascriptBridge loaded the javascript source
...
#import "JMWebViewJavascriptBridgeJS.h"

NSString * JMWebViewJavascriptBridgeJS() {
...
;(function() {
	...
	if (!window.onerror) {
		window.onerror = function(msg, url, line) {
			console.log("JMWebViewJavascriptBridge: ERROR:" + msg + "@" + url + ":" + line);
		}
	}
	...
	function _dispatchMessageFromObjC(messageJSON) {
		...
		function _doDispatchMessageFromObjC() {
			...
			if (message.responseId) {
				...
				if (!handler) {
					console.log("JMWebViewJavascriptBridge: WARNING: no handler for message from ObjC:", message);
				} else {
	...
	function _callWVJBCallbacks() {
		...
		for (var i=0; i<callbacks.length; i++) {
			callbacks[i](JMWebViewJavascriptBridge);
		}
	}
})();
