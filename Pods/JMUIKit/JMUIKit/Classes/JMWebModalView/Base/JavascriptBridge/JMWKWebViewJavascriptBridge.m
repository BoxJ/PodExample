...
@implementation JMWKWebViewJavascriptBridge {
...
+ (void)enableLogging { [JMWebViewJavascriptBridgeBase enableLogging]; }

+ (instancetype)bridgeForJMWebView:(WKWebView*)webView {
    JMWKWebViewJavascriptBridge* bridge = [[self alloc] init];
...
- (void)clearHandler {
    [_base.messageHandlers removeAllObjects];
}
...
- (void)setJMWebViewDelegate:(id<WKNavigationDelegate>)webViewDelegate {
...
- (void)WKFlushMessageQueue {
    [_webView evaluateJavaScript:[_base webViewJavascriptFetchQueyCommand] completionHandler:^(NSString* result, NSError* error) {
        if (error != nil) {
            JMLog(@"JMWebViewJavascriptBridge: WARNING: Error when trying to fetch data from WKWebView: %@", error);
        }
