//
//  JMBoomSDKBusiness+AccountUIPhoneChange.m
//  JMBoomSDK
//
//  Created by Thief Toki on 2021/2/4.
//

#import "JMBoomSDKBusiness+AccountUIPhoneChange.h"
...
@implementation JMBoomSDKBusiness (AccountUIPhoneChange)

- (void)showPhoneChangeView {
    ...
    switch (JMBoomSDKBusiness.info.registerType) {
        case JMBoomRegisterType_Phone: {
            ...
            [self checkRealNameStatusWithResponder:[JMResponder success:^(NSDictionary *info) {
                //用户已实名
                [weakSelf phoneChangeUnderVerify];
...
- (void)phoneChangeUnderVerify {
    [self phoneChangeVerifyWithCallback:^(NSDictionary * _Nullable responseObject, NSError * _Nullable error) {
        if (error) {
            [JMRktDialog showError:error];
        } else {
            [[JMBoomSDKBusiness.web initChangePhoneWithVerifyCodeLength:JMBoomSDKBusiness.config.codeLength]
